	...
	initEvent: function(target){
		...
		$target.find(".lp-contextmenu>li").on("click", function(e){
			...
			if (index == 0){
				...
				/**
				 * @author ququ
				 * @content 打开所有视频后先判断是否有视频数据，如果有则加载进度条，反之则不加载
				 */
				var hasVideo = false;
				$target.data("player").container.find("video").each(function(index, el){
					var $el = $(el);
	...
	closeVideo: function(video, callback){
		if ($(playerTarget).data("player").options.playType == "html5") {
			...
			if ($video.attr("data-video")){
				...
				/**
				 * @author ququ
				 * @content 判断下有没有还在等待重新打开视频的timeout 有则停掉
				 */
				if ($video.data("timeout")) {
					window.clearTimeout($video.data("timeout"));
				}
		...
		if ($video.attr("data-video")){
			...
			flvPlayer.on(flvjs.Events.ERROR, function(err, errDetail){
				...
				if (!playFlag && count > 6){
					...
					$video.data("timeout") = setTimeout(function() {//保存timeout
	...
	setTimeout: function(target){
		...
		$target.data("player").timer = setInterval(function(){
			...
			try {
				...
				/**
				 * @author ququ
				 * @content //倒计时开始后再显示进度条
				 */
				if (time === options.timeout || time === 60) {
					$progress.show();
				}
				/**
				 * end
...
$.fn.lavaPlayer.defaults = {
	...
	onDisconnected: null //视频断开时触发， 用于flash()
};
